...
use db::Database;
use dotenv::dotenv;
use handlers::{blog_handler::{create_blog_post, delete_blog_post_by_id, get_blog_post_by_id, get_blog_posts, get_latest_posts, get_total_posts_count, update_blog_post_by_id}, category_handler::{create_category, delete_category_by_id, get_categories, get_category_by_id, get_total_categories_count}, health_check, message_handler::{create_message, get_message_by_id}};
...
async fn load_router(app_state: Arc<Database>, cors: CorsLayer) -> Router {
    Router::new()
        ...
        .route("/api/messages/:id", get(get_message_by_id))
