use crate::{
    models::dtos::{ApiResponse, BlogPostDTO, CategoryDTO, ContactFormDTO, WebsiteInfoDTO},
    prelude::*,
};
...
pub async fn get_website_info() -> ApiResponse<WebsiteInfoDTO> {
    ...
    client
        .get(format!("{API_URL}/website_info"))
        .header("API_KEY", API_KEY)
        .send()
        .await
        .unwrap()
...
pub async fn get_posts(
    page: usize,
    page_size: usize,
) -> Result<ApiResponse<Vec<BlogPostDTO>>, reqwest::Error> {
    let http_client = reqwest::Client::new();

    http_client
        .get(format!("{API_URL}/blog"))
        .header("API_KEY", API_KEY)
...
pub async fn get_latest_posts() -> ApiResponse<Vec<BlogPostDTO>> {
    ...
    http_client
        .get(format!("{API_URL}/blog/latest_posts"))
        .header("API_KEY", API_KEY)
        .send()
        .await
        .unwrap()
...
pub async fn get_blogpost(blog_post_id: String) -> ApiResponse<BlogPostDTO> {
    ...
    http_client
        .get(format!("{API_URL}/blog/{blog_post_id}"))
        .header("API_KEY", API_KEY)
        .send()
        .await
        .unwrap()
